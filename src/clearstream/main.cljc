(ns clearstream.main
  "Kotodama WASM entrypoint for the Clearstream clean-room actor (L4) — Clojure port.
  ...
  facts (`clearstream.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "clearstream")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"      :plural "accounts"      :id-prefix "clearstr_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"         :plural "loans"         :id-prefix "clearstr_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"clearstream.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "clearstream-compat" :tier tier :entities entities} 200])
...
(defn start! [] :clearstream-compat/ready)
